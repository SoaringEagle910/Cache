...
(
    ...
    logic dcacop_op_en;
    assign dcacop_op_en = dcache_inst.is_cacop & dcache_inst.cacop_code[2:0] == 3'b001;
    ...
    always_comb begin
        case (current_state)
            `IDLE: begin
                if(dcacop_op_en & !hit_fail & !tlb_stall_mem)next_state = `DCACOP;
                else if (!pre_valid||tlb_stall_mem) next_state = `IDLE;
            ...
            `DCACOP: begin
                if(dcache_inst.cacop_code[4:3]==2'b10 && tlb_stall_mem)next_state = `DCACOP;
                else next_state = `IDLE;
            end
    ...
    assign next_is_idle=(((current_state==`IDLE&&(!pre_valid||(!dcache2transaddr.uncache&&hit_success&&pre_op==1'b0)))
                    ||(current_state==`RETURN)||((current_state==`UNCACHE_RETURN)&&(ducache_rvalid_o||ducache_bvalid_o)))&&!tlb_stall_mem)
                    ||((current_state==`DCACOP)&&!(dcache_inst.cacop_code[4:3]==2'b10 && tlb_stall_mem))||dcache2transaddr.tlb_exception;

    assign dcache2transaddr.store = pre_op & (next_state != `DCACOP);
    assign dcache2transaddr.cacop_op_mode_di = !((next_state ==`DCACOP) && dcache_inst.cacop_code[4:3]==2'b10);
    assign dcache2transaddr.data_fetch = (mem2dcache.valid && next_state==`IDLE) || (((current_state==`IDLE)&&(next_state==`DCACOP)) && dcache_inst.cacop_code[4:3]==2'b10);
    assign dcache2transaddr.data_vaddr = (next_state == `DCACOP) && dcache_inst.cacop_code[4:3] == 2'b10 ? dcache_inst.addr : (tlb_stall_mem ? pre_vaddr : mem2dcache.virtual_addr);
    ...
    assign mem2dcache.dcacop_ok = (current_state == `DCACOP) && (next_state == `IDLE);
